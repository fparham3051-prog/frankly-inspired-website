/* ==========================================================================
   Frankly Inspired — Financial Health Score
   ...
   - Revenue Concentration: multiple sources cite >30% from one source as
     ...
     15-20%. This tool treats LOWER concentration as healthier.
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var COMPOSITE_SUMMARY = [
    "This organization's core financial indicators point to real near-term risk. That's not unusual, especially for younger or fast-growing organizations, but it does mean financial stabilization is worth prioritizing before taking on major new commitments.",
    'The foundation has real gaps in more than one area below. These are addressable, but likely need deliberate attention rather than incremental fixes alone.',
    'The organization meets or is close to commonly cited minimums across most indicators. The opportunity now is tightening up the specific areas below rather than an overall rebuild.',
    'This is a financially healthy organization by sector benchmarks. The focus shifts from stabilization to protecting and strategically deploying that strength.',
    'This organization is in a genuinely strong financial position relative to sector norms. The conversation here is about strategic use of that strength, not risk mitigation.'
  ];
  ...
  var METRICS = [
    ...
    },
    {
      ...
      format: function (n) { return Math.round(n) + '%'; },
      points: [
        "Over 50% of revenue from one source is high-risk by every commonly cited benchmark — losing that single funder would be an existential event, not a budget problem. This is usually the highest-priority conversation on this page.",
        "35-50% concentration exceeds the widely cited 30% risk threshold. Diversification should be an active strategy, not a someday goal — donor retention and stewardship is often the fastest lever, since it compounds over years rather than requiring a whole new funding relationship.",
        "25-35% is close to the commonly cited risk threshold. Worth tracking this ratio annually so it doesn't drift upward unnoticed as the top funder's support grows alongside everything else.",
        "15-25% is a solid diversification position, near the stricter single-funder guidance some sources recommend.",
        "Under 15% from any single source is a strong, resilient revenue base — well-positioned to weather the loss of any one funder without an existential budget crisis."
      ]
    },
    {
      key: 'liquidity',
      ...
      points: [
        ...
        "Above 3.0 is a very strong liquidity position. Worth checking whether some of that short-term cash could be better deployed — into the operating reserve specifically, or toward mission investment — rather than sitting idle."
      ]
    }
  ];
  ...
  var status = document.getElementById('fh-status');
  var resultsEl = document.getElementById('fh-results');
  var metricsEl = document.getElementById('fh-metrics');
  ...
  function renderResults() {
    var v = {
      cash: getNum('fh-cash'),
      totalExpenses: getNum('fh-total-expenses'),
      programExpenses: getNum('fh-program-expenses'),
      totalRevenue: getNum('fh-total-revenue'),
      topSource: getNum('fh-top-source'),
      currentAssets: getNum('fh-current-assets'),
      currentLiabilities: getNum('fh-current-liabilities'),
    };
    ...
    document.getElementById('fh-gauge-score').textContent = composite;
    document.getElementById('fh-gauge-band-name').textContent = BAND_NAMES[compositeBand] + ' (' + BAND_SUBNAMES[compositeBand] + ')';
    document.getElementById('fh-summary').textContent = COMPOSITE_SUMMARY[compositeBand];
    ...
    var recommendationEl = document.getElementById('fh-recommendation');
    if (recommendationEl) {
      ...
      var tierReason = compositeBand <= 1
        ? 'with financial gaps across more than one area, hands-on weekly implementation tends to stabilize things faster than periodic advisory alone.'
        : 'with a stable-to-strong financial foundation, experienced strategic guidance is usually enough to sharpen and protect it further.';
  ...
  var retakeBtn = document.getElementById('fh-retake');
  if (retakeBtn) {
